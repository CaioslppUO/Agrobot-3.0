...
#Desinstala o utils
def uninstall_lidar_utils() -> bool:
    try:
        if(os.path.exists(catkin_ws_directory + "src/lidar/src/.dist/")):
            os.system("cd " + catkin_ws_directory + "src/lidar/src/.dist/ && yes | python3 -m pip uninstall robot_utils-0.0.1-py3-none-any.whl")
        try:
        ...
        do_log("<uninstall.py> [ERROR] Could not remove lidar_utils. "+str(e))
        return False

## Executa as rotinas de desinstalação.
def uninstall():
    global uninstalled
    uninstalled = uninstall_lidar_utils()
